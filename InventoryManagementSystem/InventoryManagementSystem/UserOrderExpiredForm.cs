...
{
    public partial class UserOrderExpiredForm : Form
    {
        ...
        private void dgvUserOrder_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Return")
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    cm.Parameters.AddWithValue("@loginfo", ("NOTE: Client  \"" + UserLoginForm.CName.ToString() + " \"  with Activity ID # " + dgvUserOrder.Rows[e.RowIndex].Cells[1].Value.ToString()) + "  RETURNED an EXPIRED request . Please Check the Returned List for Confirmation");
            ...
            if (colName == "Extend")
            {
                ...
                if (result == DialogResult.Yes)
                {
                    UserOrderExpiredModuleForm userorderexpiredmoduleform = new UserOrderExpiredModuleForm();

                    userorderexpiredmoduleform.lblOrderId.Text = dgvUserOrder.Rows[e.RowIndex].Cells[1].Value.ToString();
                    userorderexpiredmoduleform.txtReturnOrder.Text = dgvUserOrder.Rows[e.RowIndex].Cells[8].Value.ToString();
                    userorderexpiredmoduleform.dtExtendOrder.Text = DateTime.Now.ToString();
                    userorderexpiredmoduleform.ShowDialog();
                    LoadUserOrder();
                }
            }
        }
