...
{
    public partial class OrderNotApprovedForm : Form
    {
        ...
        private void dgvOrder_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to delete this activity?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm.Parameters.AddWithValue("@loginfo", ("Admin  \" " + LoginForm.adminfullname.ToString() + "\"  has been DELETED the NOT APPROVED Client  \" " + dgvOrder.Rows[e.RowIndex].Cells[6].Value.ToString() + "\"  request with Activity ID #" + dgvOrder.Rows[e.RowIndex].Cells[1].Value.ToString()));
            ...
            {
                DialogResult result = MessageBox.Show("Do you Want to Cancel Not Approve this Activity?", "Approve Activity", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("INSERT INTO tblog(loginfo, logdate)VALUES(@loginfo, @logdate)", con);
                    cm.Parameters.AddWithValue("@loginfo", ("Admin  \" " + LoginForm.adminfullname.ToString() + "\"  has been MOVED NOT APPROVED Client  \" " + dgvOrder.Rows[e.RowIndex].Cells[6].Value.ToString() + "\"  request to WAITING LIST with Activity ID #" + dgvOrder.Rows[e.RowIndex].Cells[1].Value.ToString()));
                    cm.Parameters.AddWithValue("@logdate", DateTime.Now);
                    con.Open();
                    cm.ExecuteNonQuery();
                    con.Close();
