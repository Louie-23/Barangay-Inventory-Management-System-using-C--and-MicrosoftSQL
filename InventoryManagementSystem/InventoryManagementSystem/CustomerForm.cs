...
namespace InventoryManagementSystem
{
    public partial class CustomerForm : Form
    {
        ...
        public void LoadCustomer()
        {
            ...
            cm = new SqlCommand("SELECT * FROM tbCustomer WHERE CONCAT(cid, cname, cphone, cemail, caddress, cusername, cpassword)LIKE '%"+txtSearch.Text+"%'",con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvCustomer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Edit")
            {
                ...
                customerModule.txtCEmail.Text = dgvCustomer.Rows[e.RowIndex].Cells[4].Value.ToString();
                customerModule.txtCAddress.Text = dgvCustomer.Rows[e.RowIndex].Cells[5].Value.ToString();
                customerModule.txtCUserName.Text = dgvCustomer.Rows[e.RowIndex].Cells[6].Value.ToString();
                customerModule.txtCPassword.Text = dgvCustomer.Rows[e.RowIndex].Cells[7].Value.ToString();

                customerModule.btnSave.Enabled = false;
                customerModule.btnUpdate.Enabled = true;
                customerModule.txtCUserName.Enabled = false;
                customerModule.ShowDialog();
            }
            else if (colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to delete this client?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    con.Open();
                    ...
                    cm.Parameters.AddWithValue("@loginfo", ("Admin  \" " + LoginForm.adminfullname.ToString() + "\"  DELETED a Client with a name of  \" " + dgvCustomer.Rows[e.RowIndex].Cells[2].Value.ToString() + " \"  with Client ID # " + dgvCustomer.Rows[e.RowIndex].Cells[1].Value.ToString()));
